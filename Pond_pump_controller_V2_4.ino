/*
  ...
  Features:
   ...
   - Added long press on display to prevent accidental change of mode
...
unsigned long touchDownMs = 0;          // when the finger first touched
bool touchPending = false;              // finger is currently held down
const unsigned long longPressMs = 1000; // required hold duration
...
void loadPumpModes() {
  ...
  if (m1 < MODE_ON || m1 > MODE_AUTO) m1 = MODE_AUTO;
  if (m2 < MODE_ON || m2 > MODE_AUTO) m2 = MODE_AUTO;
...
void cycleMode(int i) {
  ...
  switch (m) {
    ...
    case MODE_AUTO: m = MODE_ON;   break;
  }
  pumpMode[i] = m;
  savePumpModes();
}
...
void setup() {
  ...
  tft.init();
  tft.invertDisplay(true);  // try true, or false if already inverted
  tft.setRotation(3);
...
void loop() {
  ...
  // Long-press touch handling: finger must be held for longPressMs before mode cycles.
  // A visual countdown bar is drawn while the press is building up.
  if (ts.touched()) {
    TS_Point p = ts.getPoint();
    if (!touchPending) {
      // Finger just went down — record position and time
      touchPending = true;
      touchDownMs = millis();
      lastTouchMs = millis();
    } else if (millis() - touchDownMs >= longPressMs) {
      // Long press threshold reached — act once then reset
      touchPending = false;
      bool changed = false;
      ...
      tft.fillRect(0, rectY - 6, SCREEN_W, 4, lastDaytime ? DAY_BG : NIGHT_BG);
    } else {
      // Draw a progress bar growing from left to right under the pump rects
      unsigned long held = millis() - touchDownMs;
      int barW = (int)((held * SCREEN_W) / longPressMs);
      tft.fillRect(0, rectY - 6, barW, 4, TFT_YELLOW);
      tft.fillRect(barW, rectY - 6, SCREEN_W - barW, 4, lastDaytime ? DAY_BG : NIGHT_BG);
    }
  } else {
    if (touchPending) {
      // Finger lifted before long press — clear bar and cancel
      touchPending = false;
      tft.fillRect(0, rectY - 6, SCREEN_W, 4, lastDaytime ? DAY_BG : NIGHT_BG);
    }
  }
